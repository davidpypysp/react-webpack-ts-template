const WebrtcRos = (function () {
    ...
    WebrtcRosConnection.prototype.connect = function () {
        console.info("webrtc connection");
        const self = this;
        this.close();
        ...
        console.info("peerConnConfig", this.peerConnectionConfiguration);
        ...
        this.peerConnection.onicecandidate = function (event) {
            console.info("onice candidate", event);
            if (event.candidate) {
                const candidate = {
        ...
        this.peerConnection.ontrack = function (event) {
            ...
            const callbackData = self.addStreamCallbacks[event.streams[0].id];
            if (callbackData) {
    ...
    WebrtcRosConnection.prototype.onSignalingMessage = function (e) {
        ...
        if (dataJson.type === "offer") {
            console.log(
                "Received WebRTC offer via WebRTC signaling channel",
                dataJson
            );
            ...
            console.log(
                "Received WebRTC ice_candidate via WebRTC signaling channel",
                dataJson
            );
    ...
    WebrtcRosConnection.prototype.sendAnswer = function () {
        ...
        this.peerConnection.createAnswer(
            function (sessionDescription) {
                ...
                console.info("sendAnswer", sessionDescription);
    ...
    WebrtcRosConnection.prototype.addRemoteStream = function (config) {
        ...
        return new Promise(function (resolve, reject) {
            console.info("add remote stream promise return");
    ...
    WebrtcRosConnection.prototype.sendConfigure = function () {
        const self = this;
        const currentLastConfigureActionPromise = this
            .lastConfigureActionPromise;
        this.lastConfigureActionPromise = Promise.resolve([]);
